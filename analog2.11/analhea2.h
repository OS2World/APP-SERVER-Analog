/*** analhea2.h; header file for analog version 2.1 ***/
...
#define VNUMBER "2.11"    /* the version number of this program */
...
#define NOLNGSTRS (149) /* how many lngstr's */

enum {weekbeg_, month_, day_, days_, hr_, minute_, minutes_, second_, seconds_,
	byte_, bytes_, request_, requests_, date_, file_, host_, dir_, dom_,
	ext_, url_, browser_, nr_, description_, webstatsfor_, monthrep_,
	weekrep_, daysum_, dayrep_, hourrep_, hoursum_, hostrep_, hostgs_,
	hostgp_, hostgen_, dirrep_, dirgs_, dirgp_, dirgen_, typerep_, extgs_,
	extgp_, extgen_, reqrep_, filegs_, filegp_, filegen_, refrep_, refgs_,
	refgp_, refgen_, browsum_, browgs_, browgp_, browgen_, browrep_,
	domrep_, domgs_, domgp_, domgen_, subdomgs_, subdomgp_, subdomgen_,
	credit_, runtime_, lessone_, dirlevel_, eachunit_, represents_,
	partof_, pagereq_, pagereqs_, statrep_, errrep_, allerrs_, allerrs1_,
	allerrsn_, occsort_, errtype_, progstart_, localtime_, reqstart_, to_,
	succreqs_, avereqs_, totpages_, avepages_, totfails_, totredirs_,
	distfiles_, disthosts_, approxhosts_, newhosts_, approxnewhosts_,
	corrupt_, unwanted_, totdata_, avedata_, brackets_, sevendaysto_,
	lastsevendays_, goto_, top_, nreqs_, preqs_, noccs_, poccs_, npgs_,
	ppgs_, nbytes_, pbytes_, firstsm_, firstsf_, firstsn_, firstdsm_,
	firstdsf_, firstdsn_, allsm_, allsf_, allsn_, atleast_, ptraffic_,
	btraffic_, sortedms_, sortedfs_, sortedns_, sortedmp_, sortedfp_,
	sortednp_, traffic_, ppages_, npages_, prequests_, nrequests_,
	alphasortms_, alphasortfs_, alphasortns_, alphasortmp_,	alphasortfp_,
	alphasortnp_, unsortedms_, unsortedfs_, unsortedns_, unsortedmp_,
	unsortedfp_, unsortednp_, colon_, datefmt0_, datefmt1_,	datefmt2_};
...
#ifndef NODNS
extern flag dnsresolve(char *hostn);
#endif
...
extern void allaliases(struct genstruct **objhead,
  struct genstruct **objhead2, size_t hashsize, int *totalobjs,
...
/* in hash.c */
extern struct genstruct *hashadd(struct genstruct **objhead, size_t hashsize,
...
extern struct genstruct *gensort(struct genstruct **objhead, size_t hashsize,
...
extern void pvfiletest(char *name);
...
extern size_t htmlstrlen(char *s);
extern void matchlength(FILE *outf, char *s, char c);
extern size_t datefmtlen(char *fmt);
extern void dateprintf(FILE *outf, char *fmt, int date, int month, int year,
  int hr, int min);
...
extern void printcolheads(FILE *outf, char *wantcols, int fieldwidth,
  int pfieldwidth, int bfieldwidth, char bprefix[2], char name[20], int width,
...
extern void datehead(FILE *outf, int maxreq, int maxpages, double maxbytes,
  ...
  char colhead[13], int colwidth, char codeletter, int *unit,
...
extern NP_type checkValue(char *typeStr, const MatchPairs table[]);
extern NP_type checkType(char *typeStr);
extern void build_agent(char agent[MAXSTRINGLENGTH], NP_type entryType,
  char *entryTypeStr, NP_type actionType, char *actionTypeStr);
extern int readDigits(char **from);
extern int scan_path(char *inputline, char filename[MAXSTRINGLENGTH],
  int maxlen, struct include *expandhead);
extern int sscanf_netpresenz(FILE *logfile, char *inputline,
